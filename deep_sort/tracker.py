...
class Tracker(object):
    ...
    kf : List[kalman_filter.KalmanBoxTracker]
        The list of KalmanBoxTracker to track detected objects in image space.
    tracks : List[Track]
    ...
    def _initiate_track(self, detection):
        '''
        Initialize a tracker for a detection
        '''
        # initialize a kalman box tracker to track the given detection
        kft = kalman_filter.KalmanBoxTracker()
        mean, covariance = kft.initiate(detection.to_xyah())
        self.kf.append(kft)
        
        class_name = detection.get_class()
        
        self.tracks.append(Track(mean, covariance, self._next_id, self.n_init, 
                                 self.max_age, detection.feature, class_name))
        self._next_id += 1
